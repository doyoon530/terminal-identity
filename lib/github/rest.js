...
async function fetchGithubRestStats(username, options = {}) {
  const normalized = normalizeUsername(username);
  if (!normalized) {
    return null;
  }
  ...
  const languageBytes = needsLangs ? await fetchGithubLanguageBytes(ownedSourceRepos) : null;
  const topLangsSource = needsLangs ? languageBytes || primaryLanguageCounts : null;
  const topLangs = topLangsSource
    ? Object.entries(topLangsSource)
        .sort(([, a], [, b]) => b - a)
        .slice(0, 12)
        .map(([name, count]) => ({ name, count }))
    : null;

  const contributions = needsContribs ? await fetchGithubContributions(normalized) : null;
  const avatarDataUri = needsAvatar
    ? await fetchImageDataUri(withImageSize(String(user.avatar_url || ""), 120))
    : null;
  ...
  return {
    ...
    stars,
    forks,
    topLangs: topLangs?.length ? topLangs : null,
    contributions,
  };
}
